...
namespace FolderMusicDebug
{
    ...
    {
        private const int maxDebugEventStringLength = 100000, maxEventCount = 300;
        ...
        public static ViewModelDebug Current
        {
            get
            {
                if (instance == null) instance = new ViewModelDebug();
        ...
        private async Task LoadDebugEvents()
        {
            try
            {
                string dataEventsString = await PathIO.ReadTextAsync(DebugDataFilepath);

                if (dataEventsString.Length > maxDebugEventStringLength)
                {
                    int removeLength = dataEventsString.Length - maxDebugEventStringLength;
                    dataEventsString = dataEventsString.Remove(0, removeLength);
                }

                Events = DebugEvent.GetEvents(dataEventsString).ToList();

                string message = "DataStringLength: " + dataEventsString.Length + "\nEventsCount: " + Events.Count;
        ...
        private async Task ReduceEvents()
        {
            if (Events.Count < maxEventCount) return;

            IEnumerable<DebugEvent> events = Events.Skip(Events.Count - maxEventCount);

            await PathIO.WriteTextAsync(DebugDataFilepath, events.GetDataEventsString());
        }
