    ...
    class BackgroundCommunicator
    {
        public static void SendPlaylistsAndSongsIndexAndShuffleIfComplete(Playlist playlist)
        {
            ...
            if (playlist.Shuffle == ShuffleKind.Complete)
            {
                ...
                valueSet.Add("ShuffleList", XmlConverter.Serialize(playlist.ShuffleSongs));
            }
        ...
        private static void MessageReceivedFromBackground(object sender, MediaPlayerDataReceivedEventArgs e)
        {
            ...
            foreach (string key in valueSet.Keys)
            {
                switch (key)
                {
                    ...
                    case "Pause":
                        Library.ViewModel.UpdatePlayPauseIconAndText();
                        return;
        ...
        private static void GetSongsIndex(ValueSet valueSet)
        {
            ...
            if (songsIndex != Library.Current.CurrentPlaylist.SongsIndex)
            {
                Library.ViewModel.CurrentPlaylist.SongsIndex = songsIndex;
                Library.ViewModel.CurrentPlaylist.UpdateCurrentSong();
            }
        }
        ...
        private static void GetSongsIndexAndShuffle(ValueSet valueSet)
        {
            ...
            {
                Library.ViewModel.CurrentPlaylist.SongsIndex = songsIndex;
                Library.ViewModel.CurrentPlaylist.Shuffle = shuffle;
                Library.ViewModel.CurrentPlaylist.ShuffleList = shuffleList;

                Library.ViewModel.CurrentPlaylist.UpdateSongsAndShuffleListSongs();
                Library.ViewModel.CurrentPlaylist.UpdateCurrentSong();
                Library.ViewModel.CurrentPlaylist.UpdateShuffleIcon();
            }
        }
